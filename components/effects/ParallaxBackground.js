  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      planesRef.current.forEach((plane, index) => {
        ...
        const baseProgress = ((elapsed + (planeConfig.delay * 1000)) % (planeConfig.duration * 1000)) / (planeConfig.duration * 1000);
        ...
        switch(planeConfig.altitude) {
          case 'high':
            ...
            y = Math.sin(baseProgress * Math.PI * 3) * 15 + Math.sin(elapsed * 0.0002) * 5;
            rotation = Math.atan2(Math.cos(baseProgress * Math.PI * 3), -Math.sin(baseProgress * Math.PI * 2)) * (180 / Math.PI);
            break;
          case 'medium':
            x = Math.sin(baseProgress * Math.PI * 3) * 60;
            y = Math.cos(baseProgress * Math.PI * 2) * 25 + Math.sin(elapsed * 0.0003) * 8;
            rotation = Math.atan2(Math.sin(baseProgress * Math.PI * 2), Math.cos(baseProgress * Math.PI * 3)) * (180 / Math.PI);
            break;
          case 'low':
            x = Math.sin(baseProgress * Math.PI * 2) * 50;
            y = Math.cos(baseProgress * Math.PI * 4) * 20 + Math.sin(elapsed * 0.0004) * 10;
            rotation = Math.atan2(Math.cos(baseProgress * Math.PI * 4), Math.sin(baseProgress * Math.PI * 2)) * (180 / Math.PI);
            break;
        }
  ...
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none">
      <div ref={containerRef} className="w-full h-full">
          ...
          <svg width="100%" height="100%" className="opacity-30">
            <defs>
              <linearGradient id="ellipseGradient" x1="0%" y1="0%" x2="100%" y2="100%">
                <stop offset="0%" style={{ stopColor: 'rgb(120, 87, 255)', stopOpacity: 0.3 }} />
        ...
        {/* Cloud Layers */}
        <div className="absolute inset-0">
          <div className="absolute inset-0 bg-gradient-to-b from-transparent via-accent-purple/5 to-transparent animate-float" 
            style={{ animationDuration: '20s' }} 
          />
          <div className="absolute inset-0 bg-gradient-to-t from-transparent via-accent-blue/5 to-transparent animate-float" 
        ...
        <div className="absolute inset-0">
          ...
          {[...Array(30)].map((_, i) => (
            <div
              ...
              className={`absolute rounded-full animate-float ${
                i % 3 === 0 ? 'bg-accent-purple/15' : 
                i % 3 === 1 ? 'bg-accent-blue/15' : 
                'bg-accent-pink/15'
              }`}
              style={{
          ...
          {[...Array(50)].map((_, i) => (
            <div
              key={`medium-${i}`}
              className={`absolute rounded-full animate-float ${
                i % 3 === 0 ? 'bg-accent-purple/10' : 
                i % 3 === 1 ? 'bg-accent-blue/10' : 
                'bg-accent-pink/10'
              }`}
              style={{
          ...
          {[...Array(100)].map((_, i) => (
            <div
              key={`small-${i}`}
              className={`absolute rounded-full animate-float ${
                i % 3 === 0 ? 'bg-accent-purple/5' : 
                i % 3 === 1 ? 'bg-accent-blue/5' : 
                'bg-accent-pink/5'
              }`}
              style={{
                width: '1px',
